package com.miniweverse.common.openapi;
 ...
 *
 * ErrorResponse 스키마를 서비스별 OpenAPI 빈의 components에 직접 넣으면 springdoc이 최종
...
public class CommonOpenApiConfig {
        ...
        return openApi -> {
            ...
            openApi.getPaths().values().forEach(pathItem ->
                    pathItem.readOperations().forEach(operation -> {
                        ...
                        addIfAbsent(responses, CommonErrorCode.VALIDATION_ERROR);
                        if (operation.getSecurity() != null && !operation.getSecurity().isEmpty()) {
                            addIfAbsent(responses, CommonErrorCode.UNAUTHORIZED);
                        }
                        addIfAbsent(responses, CommonErrorCode.INTERNAL_SERVER_ERROR);
                    }));
        };
    }

    private void addIfAbsent(ApiResponses responses, CommonErrorCode errorCode) {
        ...
        example.put("error", Map.of("code", errorCode.getCode(), "message", errorCode.getMessage()));
    ...
    private Schema<?> errorResponseSchema() {
        Schema<?> error = new ObjectSchema()
                .addProperty("code", new StringSchema().example(CommonErrorCode.VALIDATION_ERROR.getCode()))
                .addProperty("message", new StringSchema().example(CommonErrorCode.VALIDATION_ERROR.getMessage()));
