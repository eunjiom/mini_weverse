...
<details>
<summary>멤버십 변경 알림 (동기 호출 vs 트랜잭션 아웃박스 + Kafka)</summary>
...
| **아웃박스 + Kafka 발행 (선택)** | chat-service 장애/다운타임과 무관하게 이벤트 유실 없음 — 브로커가 메시지를 보존하고 있다가 chat-service가 복구되면 자동으로 이어받아 처리 | 아웃박스 테이블·발행자·폴링 로직에 더해 Kafka 인프라·컨슈머 관리 필요, 즉시 반영은 아니고 최대 폴링 주기(5초)만큼 지연 |

**선택 이유**: chat-service의 멤버십 상태(`MembershipPeriod`/`MembershipCache`)는 단순 알림이 아니라 채팅 가시성 판단에 직접 쓰이는 데이터라, chat-service 장애와 무관하게 이벤트가 유실되지 않는 게 중요하다고 판단. 알림(2.6)에 이미 같은 이유로 Kafka 인프라가 구축돼 있어 추가 인프라 비용 없이 적용 가능했음. chat-service를 강제 종료한 상태에서 이벤트를 발생시키고 재기동해, 자동으로 밀린 이벤트를 이어받아 처리하는 것까지 실측으로 확인함
