...
![아키텍처 다이어그램](image/아키텍처%20최최종.png)
...
![MSA 서비스](image/msa%20서비스%20최종.png)
...
**주요기능 설명**: 구독/만료 처리(DB 트랜잭션)와 chat-service 반영을 분리 — "상태 변경 커밋 + 이벤트 적재"까지만 원자적으로 보장하고, 실제 발행은 스케줄러가 5초마다 폴링하며 Kafka(`membership-sync-events` 토픽)로 전송. chat-service는 이 토픽을 구독해 캐시·기간 이력을 갱신한다
...
| 아웃박스 + REST 발행(처음 선택했던 방식) | Kafka 없이도 구현 가능, 즉시 반영은 아니지만 최대 폴링 주기(5초)만큼만 지연 | chat-service가 재시도 한도(5회)를 넘겨 다운되면 이벤트가 DEAD 처리되고 자동 재시도가 끊겨 운영자가 수동으로 확인해야 함 — 그동안 chat-service의 멤버십 상태가 실제와 어긋난 채로 방치됨 |
| **아웃박스 + Kafka 발행 (선택)** | chat-service 장애/다운타임과 무관하게 이벤트 유실 없음 — 브로커가 메시지를 보존하고 있다가 chat-service가 복구되면 자동으로 이어받아 처리 | 아웃박스 테이블·발행자·폴링 로직에 더해 Kafka 인프라·컨슈머 관리 필요 |

**선택 이유**: 처음엔 REST로 아웃박스를 발행했으나, chat-service의 멤버십 상태(`MembershipPeriod`/`MembershipCache`)는 단순 알림이 아니라 채팅 가시성 판단에 직접 쓰이는 데이터라 "장애와 무관한 무유실"이 실제로 보장돼야 한다고 판단. 알림(2.6)에 이미 같은 이유로 Kafka 인프라가 구축돼 있어 추가 인프라 비용 없이 전환 가능했음. 전환 후 chat-service를 강제 종료한 상태에서 이벤트를 발생시키고 재기동해 자동으로 밀린 이벤트를 이어받아 처리하는 것까지 실측으로 확인함
...
- [ ] AWS 전환 + 아웃박스 발행자 스케일아웃
  전환 시점 기준: 3.3 실측에서 구독자 2만명대부터 fan-out 지연이 20초를 넘기기 시작했으므로, 한 아티스트의 활성 구독자 수가 이 규모에 근접하면 전환. 온프레미스 단일 인스턴스로는 아웃박스 발행자를 늘릴 수 없어, AWS 전환과 발행자 다중 인스턴스화를 함께 진행할 예정
